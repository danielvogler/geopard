...
class GeopardResponse:
    ...
    def __init__(
        self, time, dtw, start_point, end_point, match_flag, error=None
    ):
        """init."""
        self.time = time
        self.dtw = dtw
...
class Geopard:
    ...
    def dtw_match(
        ...
        start_region: Optional[Polygon] = None,
        finish_region: Optional[Polygon] = None,
    ) -> TypeVar:
        ...
        Args:
            ...
            min_trkps (int, optional): Minimum number of GPS points.
                Defaults to 50.
            radius (float, optional): Radius in meters. Defaults to 7.
            dtw_threshold (float, optional): DTW threshold.
                Defaults to 0.2.
            dtw_margin_range (float, optional): DTW margin range.
                Defaults to 1.5.
        ...
        # check if gpx track [lat, lon, ele, time] is complete.
        # Missing elevation data is accepted
        for k in [0, 1, 3]:
            none_idx = [i for i in range(len(trkps[k])) if trkps[k, i] is None]

            # error if required gpx track information is missing
            if none_idx:
                srt_none = sorted(set(none_idx))[:3]
                raise GeopardException(
                    f"GPX file corrupted. Check GPX points (E.g. {srt_none})"
                )

        # search activity trackpoints matching with gold start/end
        try:
            # crop activity data to segment length
            gpx_cropped = self.gpx_track_crop(
                gold, trkps, radius, start_region, finish_region
            )

            # find potential start/end trackpoints
            _nn_start, nn_start_idx = self.nearest_neighbours(
                gpx_cropped, gold[:4, 0], radius, start_region
            )
            _nn_finish, nn_finish_idx = self.nearest_neighbours(
                gpx_cropped, gold[:4, -1], radius, finish_region
            )
        ...
        for i in nn_start_idx:
            ...
            for j in nn_finish_idx:
                # start needs to happen before finish and include
                # min_trkps in between
                if i < j - min_trkps:
        ...
        seg_sort = [
            ...
            for idx, value in sorted(
                enumerate(seg_info[0]), key=lambda x: x[1]
            )
        ]
        ...
        while final_dtw > dtw_threshold and combinations_to_test > s + 1:
            ...
            # only save dtw value and time for shortest (first) match
            # in grey zone. Shortest grey zone match is overwritten
            # if hard dtw threshold is crossed
        ...
        logging.info(
            "Total execution time: %s", str(datetime.now() - start_time)
        )

        if s > -1:
            logging.info(
                "Execution time per combination: %s",
                str((datetime.now() - start_time) / (s + 1)),
            )
    ...
    def gpx_loading(self, file_name: str) -> TypeVar:
        ...
        logging.info("Load gpx file: %s", file_name)

        # load file
        with open(file_name, "r", encoding="utf8") as gpx_data_open:
            gpx_data = parse(gpx_data_open)

        def extract_trackpoints(gpx_data: TypeVar) -> TypeVar:
            # initialize lat/lon
            trkp_lat = []
            trkp_lon = []
            trkp_ele = []
            ...
            logging.info("\tExtract relevant gpx data")
            ...
            return trkp_lat, trkp_lon, trkp_ele, trkp_time

        def find_duplicate_trackpoints(
            trkp_lat: TypeVar, trkp_lon: TypeVar
        ) -> np.ndarray:
            """Find duplicate trackpoints.

            Args:
                trkp_lat (TypeVar): Latitude data
                trkp_lon (TypeVar): Longitude data

            Returns:
                np.ndarray: Indices of duplicate tracjkpoints
            """
            logging.info("\tChecking for duplicate lat/lon trackpoints")
            duplicates_lat = [
                i
                for i in range(1, len(trkp_lat) - 1)
                if trkp_lat[i] == trkp_lat[i - 1]
                and trkp_lat[i] == trkp_lat[i + 1]
            ]
            duplicates_lon = [
                i
                for i in range(1, len(trkp_lon) - 1)
                if trkp_lon[i] == trkp_lon[i - 1]
                and trkp_lon[i] == trkp_lon[i + 1]
            ]

            # duplicate values for both lat and lon
            duplicates_lat_lon = sorted(
                list(set(duplicates_lat) & set(duplicates_lon))
            )

            return duplicates_lat_lon

        trkp_lat, trkp_lon, trkp_ele, trkp_time = extract_trackpoints(
            gpx_data=gpx_data
        )

        duplicates_lat_lon = find_duplicate_trackpoints(
            trkp_lat=trkp_lat, trkp_lon=trkp_lon
        )

        logging.info("\tRemoving redundant trackpoints")
    ...
    def gpx_track_crop(
        ...
        Args:
            ...
            start_region (TypeVar, optional): Start region of track.
                Defaults to None.
            finish_region (TypeVar, optional): Finish region of track.
                Defaults to None.
        ...
        nn_start, _nn_start_idx = self.nearest_neighbours(
            gpx_data, gold[:4, 0], radius, start_region
        )
        nn_finish, _nn_finish_idx = self.nearest_neighbours(
            gpx_data, gold[:4, -1], radius, finish_region
        )
        ...
        indices = [
            i
            for i in range(len(gpx_data[3]))
            if nn_start_earliest <= gpx_data[3, i] <= nn_finish_latest
        ]
    ...
    def nearest_neighbours(
        ...
        Args:
            ...
            centroid (TypeVar, optional): Centroid around which neighbors
                should be found. Defaults to None.
            radius (float, optional): Radius. Defaults to None.
            region (Polygon, optional): Region object. Defaults to None.
        ...
        if region is not None:
            ...
            logging.info("\tNearest neighbors: %s", len(idx))
            logging.info("\t\twithin region polygon: %s", region)

        # no polygon region given - default to circle around start
        else:
            # distance (m) of all points to centroid
            distance = [
                self.spheroid_point_distance(i, centroid[:2])
                for i in gpx_data[:4, :][:2].T
            ]

            # points within radius distance of centroid
            idx = [int(i) for i, x in enumerate(distance) if x < radius]

            logging.info("\tNN: %s", len(idx))
            logging.info("\tRadius: %.1fm", radius)
            logging.info("Centroid: %f", centroid[:2])
    ...
    def spheroid_point_distance(
        self, coordinates_1: list, coordinates_2: list
    ) -> float:
        ...
        """
        logging.debug("\t Compute distance between two points on spheroid.")

        avg_earth_radius = (
            6371008.7714  # [m] https://doi.org/10.1007/s001900050278
        )
        ...
        distance = (
            ...
            * asin(
                sqrt(
                    sin(delta_lat) ** 2
                    + cos(lat_1) * cos(lat_2) * sin(delta_lon) ** 2
                )
            )
        )
    ...
    def acm(
        self, reference, query, distance_metric: str = "euclidean"
    ) -> TypeVar:
        """Accumulated cost matrix calc.

        According to:
        (1) Müller, Meinard. Information retrieval
            for music and motion. Vol. 2.
            Heidelberg: Springer, 2007.
            https://doi.org/10.1007/978-3-540-74048-3
        """
        logging.debug("\t Compute ccumulated cost matrix")

        # cm = self.cost_matrix(reference, query)
        cm = cdist(reference, query, metric=distance_metric)

        # sequence lengths
        length_n, length_m = cm.shape
    ...
    def interpolate(self, gpx_data: TypeVar) -> TypeVar:
        ...
        data = np.array(
            [
                (
                    x + np.random.uniform(low=-1e-7, high=1e-7),
                    y + np.random.uniform(low=-1e-7, high=1e-7),
                )
        ...
        tck, u = splprep(
            data.T, u=None, s=0.0, t=10, per=0
    ...
    def plot_track_comparison(
        self, gold_file_name: str, activity_file_name: str, radius: float = 7.0
    ) -> None:
        """Plot gold and activity GPX tracks together."""
        # load gold standard/baseline segment
        gold_gpx_track = self.gpx_loading(gold_file_name)
        # interpolate gold data
        gold_interpolated = self.interpolate(gold_gpx_track)

        # load activity data to be edited
        activity_gpx_track = self.gpx_loading(activity_file_name)
        # crop activity data to segment length
        gpx_cropped = self.gpx_track_crop(
            gold_gpx_track, activity_gpx_track, radius
        )

        # find potential start/end trackpoints - just for plotting
        nn_start, _nn_start_idx = self.nearest_neighbours(
            gpx_cropped, gold_gpx_track[:4, 0], radius
        )
        nn_finish, _nn_finish_idx = self.nearest_neighbours(
            gpx_cropped, gold_gpx_track[:4, -1], radius
        )

        fig = plt.figure(
            num=None, figsize=(12, 8), dpi=80, facecolor="w", edgecolor="k"
        )
        self.gpx_plot(fig, nn_start, ["NN Start Cropped", "X", "b"], 12)
        self.gpx_plot(fig, nn_finish, ["NN Finish Cropped", "P", "b"], 12)
        self.gpx_plot(fig, activity_gpx_track, ["Activity", ".", "k"])
        self.gpx_plot(fig, gpx_cropped, ["Activity Cropped", "o", "k"])
        self.gpx_plot(fig, gold_gpx_track, ["Gold", "o", "r"])
        ...
        self.gpx_plot(fig, gpx_cropped, ["Activity Cropped", "o", "k"])
        gpx_interpolated = self.interpolate(gpx_cropped)
        self.gpx_plot(
            fig, gpx_interpolated.T, ["Activity Interpolated", ".", "k"]
        )
        self.gpx_plot(fig, gold_gpx_track, ["Gold", "o", "r"])
        self.gpx_plot(
            fig, gold_interpolated.T, ["Gold Interpolated", ".", "r"]
        )
    ...
    def parse_response(self, response: GeopardResponse) -> None:
        ...
        logging.info("Response DTW: %.3f", response.dtw)
        logging.info("Response start_point: %s", response.start_point)
        logging.info("Response end_time: %s", response.end_point)
        logging.info("Response match_flag: %s", response.match_flag)
        logging.info("Response success: %s", response.is_success)
        logging.info("Response error: %s", response.error)
